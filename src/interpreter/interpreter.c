...
int interpreter_jump_to_function(Interpreter_Instance* instance, const char* function_name)
{
    for (int i = 0; i < instance->func_count; i++) {
        if (interpreter_ci_strcmp(instance->functions[i].name, function_name) == 0) {
            ...
            instance->call_stack[instance->stack_pointer].return_var = (void*)0;
...
int interpreter_execute_chunk(Interpreter_Instance* instance, int max_instructions)
{
    ...
    if (instance->waiting_for_input) {
        if (instance->input_mode == INPUT_MODE_SERIAL) {
             if (instance->serial_bytes_to_read == -1) {
                if (com1_data_available()) {
                    while (com1_data_available() && instance->serial_bytes_read < INTERPRETER_MAX_INPUT_LENGTH) {
                    ...
                    input_value.string.size = instance->serial_bytes_read;
            ...
            if (instance->serial_bytes_to_read == -1) {
                if (usb_serial_data_available()) {
                    while (usb_serial_data_available() && instance->serial_bytes_read < INTERPRETER_MAX_INPUT_LENGTH) {
